...
def _load_rules():
    rules = yaml.safe_load((Path(__file__).resolve().parents[1] / "config" / "rules.yaml").read_text(encoding="utf-8"))
...
def extract_to_excel(text: str):
    ...
    with pd.ExcelWriter(writer_path, engine="openpyxl") as writer:
        for kind, items in ents.items():
            pd.DataFrame({"valor": items}).to_excel(writer, index=False, sheet_name=(kind[:31] or "items"))
